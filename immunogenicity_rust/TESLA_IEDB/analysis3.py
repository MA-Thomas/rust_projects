...
def load_and_preprocess_data(args):
    ...
    # return hla_df
    return combined_tesla_data.sort_values(by='peptides'), combined_iedb_data.sort_values(by='peptides')
...
if __name__ == "__main__":
    ...
    tesla_df, iedb_df = load_and_preprocess_data(args)
    ...
    with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
        futures = {executor.submit(process_row, idx, row, args, start_time): idx for idx, (df_index,row) in enumerate(tesla_df.iterrows())}
    ...
    with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
        futures = {executor.submit(process_row, idx, row, args, start_time): idx for idx, (df_index,row) in enumerate(iedb_df.iterrows())}
