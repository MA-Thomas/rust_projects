...
if __name__ == "__main__":
    ...
    for idx, (df_index,row) in enumerate(hla_df.iterrows()):
        ...
        gamma_logkd_nonself_values_master = sorted(list(set( list(np.round(create_log_spaced_list(1e-2, 1.0, 5),4)) + list(np.round(create_log_spaced_list(5e-3, 0.6, 10),4))  +[1e-8, 1e-100])))
        
        for glgkd_nonself in gamma_logkd_nonself_values_master:
            # glgkd_nonself = gamma_logkd_nonself_values_master[0]

            gamma_logkd_nonself_values = [glgkd_nonself]
            #################################################################################################
            #################################################################################################
            ##                                 Self settings.
    ...
    ##
            save_pkl_dir = args.save_pkl_dir + '/d_ub_'+str(d_ub)+'_d_lb_'+str(d_lb)+'/'+allele+'/gamma_logKd_nonself_'+str(glgkd_nonself)
            if not os.path.exists(save_pkl_dir):
                os.makedirs(save_pkl_dir)
